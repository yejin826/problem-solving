...
3 try : line 79, default value 잘못 설정
	...
	사실 이문제는 BFS를 한 번 만 사용해도 해결할 수 있었다!
	내 풀이를 확신할 수 없었어서 다른 사람들의 풀이를 봤는데 BFS를 두 번 사용했었다.
	그러나 입력되는 N의 값이 작기 때문에 완탐을 돌아도 시간 복잡도는 O(N*N)이다.
	따라서 문제 풀이 순서는 
	1) 섬마다 번호를 부여한다
	2) 번호가 다른 섬인 육지마다 거리를 구한다.
